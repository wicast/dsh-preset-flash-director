/**
 * dsh-preset-flash-director — 预设自动同步（纯异步函数，可单测）
 *
...
async function updatePreset(target, marker, fault) {
  ...
  try {
    ...
    if (currentBaseline !== null && bundledBaseline !== null && marker.b !== null && sha256(currentBaseline) !== marker.b) {
      ...
      // 标记里继续记"打包基线"的指纹（而不是用户基线的）：保护必须能反复生效——
      // 若记用户基线的哈希，下一次更新时 sha256(现网) == b，会被误判成"用户没改过"，
      // 于是静默覆盖掉用户的基线（保护只生效一次）。
      installedBaselineHash = sha256(bundledBaseline)
