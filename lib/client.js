window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * dsh-preset-flash-director — Client 半
     *
     * 设置侧栏「Flash 主控」分区页：编辑 Flash 主控·Pro 专家 预设的两类配置
     *   1. 覆盖文件 expert-delegation.config.json（13 键，保存后下次委派生效）
    ...
    function insertCss() {
      ...
      if (document.getElementById("dsh-preset-flash-director-style") !== null) return;
      const tag = document.createElement("style");
      tag.id = "dsh-preset-flash-director-style";
      tag.textContent = [
    ...
    const OVERRIDE_FIELDS = [
      ...
      // fallback（opt-in）：主 pro 报错/不可用时自动降级用这套配置，并在新建的专家
      // session 里跑；失败的尝试一律退款（不计额度）。留空 = 关闭 fallback。
      { key: "expertFallbackProvider", label: "Fallback Provider（主 pro 失败后）", sub: "留空=继承专家 Provider；与主 pro 完全相同则视为未启用", type: "text", ph: "留空=继承专家 Provider", emptyLabel: "留空=继承专家 Provider" },
      { key: "expertFallbackModel", label: "Fallback 模型", sub: "主 pro 报错/不可用时的备用模型（新建 session 运行）", type: "text", ph: "留空=关闭 fallback", emptyLabel: "留空=关闭 fallback" },
      { key: "expertFallbackMaxTokens", label: "Fallback 输出上限", sub: "留空=继承专家输出上限", type: "number" },
      { key: "expertFallbackReasoningEffort", label: "Fallback 思考强度", sub: "留空=继承专家思考强度", type: "select", options: [["", "继承专家档位"], ["off", "off"], ["low", "low"], ["high", "high"], ["max", "max"]] },
    ];
    ...
    function SettingsPanel() {
      ...
      // fallback 的 provider/model 用同源数据：fallback provider 留空时 fallback 模型
      // 只在专家 Provider 的模型里选（语义 = 同 provider 换模型）。
      ...
      const renderControl = (f, get, set, isBaseline) => {
        ...
        const catalogSelect = (options) => {
          const empty = f.emptyLabel || "继承(基线/默认)";
          const opts = isBaseline ? options : [["", empty], ...options];
        ...
        if ((f.key === "expertProvider" || f.key === "expertFallbackProvider") && providers.length > 0) {
          return catalogSelect(providerOptions());
        }
        if ((f.key === "expertModel" || f.key === "expertFallbackModel") && providers.length > 0) {
          const providerKey = f.key === "expertModel" ? "expertProvider" : "expertFallbackProvider";
          return catalogSelect(modelOptionsFor(get(providerKey) || get("expertProvider")));
        }
        if (f.type === "select") {
          return react.createElement("select", {
            className: "fd-select",
      ...
      return react.createElement("div", { className: "fd-panel" }, [
        ...
        react.createElement("div", { className: "fd-card" }, [
          ...
          ...overrideFields,
          react.createElement("div", { className: "fd-note" }, "Fallback 四键：主 pro 报错或不可用时自动改用这套配置，并在新建的专家 session 里跑；失败的那次委派不计额度（自动退款）。留空 = 关闭 fallback。"),
        ...
        react.createElement("div", { className: "fd-card" }, [
          ...
          react.createElement("div", { className: "fd-note" }, "⚠ 基线改动需新开会话生效；followupRetryBudget、expertReasoningEffort 与 fallback 四键不在基线，只能走覆盖文件（运行期生效）。保存时自动备份并做 YAML 校验。"),
          ...baselineFields,
          react.createElement("div", { className: "fd-btns" }, [
            react.createElement("button", {
              type: "button",
    ...
    function SettingsCard() {
      ...
      return react.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 4 } }, [
        ...
        react.createElement("div", { className: "fd-card-sub" },
          ...
          + (eff.expertFallbackModel ? " · fallback " + eff.expertFallbackModel : " · 未配 fallback")),
        react.createElement("div", { className: "fd-card-sub" },
    ...
    function apply(ctx) {
      ...
      slots.inject("settings.section", () => slots.register(
        { name: "settings.section", id: "dsh-preset-flash-director-section", order: 30, label: "Flash 主控" },
