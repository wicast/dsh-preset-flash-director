/**
 * dsh-preset-flash-director — 配置 schema / 校验 / 归一化（纯函数层）
 *
 * 与 expert-delegation.mjs 的键集合与默认值保持同源。13 个键全可选：
 ...
 *
 * fallback 四键（expertFallbackProvider/Model/MaxTokens/ReasoningEffort）同样无默认值：
 * 不配 = 关闭"主 pro 失败自动降级" = 历史行为；它们只走覆盖文件（不进基线），
 * 因为降级是运行期运行策略，改完下次委派即生效、无需新开会话。
 */
...
/** 内置默认（与 expert-delegation.mjs 的 FALLBACK 同值；无 expertReasoningEffort，也无 fallback 四键）。 */
...
const RANGES = {
  ...
  expertFallbackMaxTokens: [1, 262144],
...
/**
 * 13 键 schemastery schema（全键 .required(false)）。
...
export function buildSchema(z) {
  return z.object({
    ...
    expertFallbackProvider: z.string().required(false),
    expertFallbackModel: z.string().required(false),
    expertFallbackMaxTokens: z.number().step(1).min(RANGES.expertFallbackMaxTokens[0]).max(RANGES.expertFallbackMaxTokens[1]).required(false),
    expertFallbackReasoningEffort: z.string().pattern(/^(off|low|high|max)$/).required(false),
  })
}
...
export function isValidKeyValue(key, value) {
  switch (key) {
    case 'expertProvider':
    case 'expertModel':
    case 'expertFallbackProvider':
    case 'expertFallbackModel':
      return typeof value === 'string' && value.trim() !== ''
    case 'expertMaxTokens':
    case 'expertFallbackMaxTokens':
    ...
    case 'expertFallbackReasoningEffort':
...
export function normalizeOverride(raw) {
  ...
  for (const key of Object.keys(raw)) {
    ...
    const value = raw[key]
    // 空值（""/null）等价于"未设置"：静默跳过，不算非法键——模板文件的空占位与
    // 设置页清空都走这条路径，不该在设置页报"忽略非法键"。
    if (value === undefined || value === null || value === '') continue
    if (isValidKeyValue(key, value)) values[key] = value
    else dropped.push(key)
  }
  return { values, dropped }
}

/** 生效值合并：override > baseline > DEFAULTS（无默认的键缺省：思考强度、fallback 四键）。 */
