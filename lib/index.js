/**
 * dsh-preset-flash-director — 服务端半
 *
 ...
 *   - 覆盖文件 = 部分覆盖（未出现的键回退基线/默认；13 键，含 fallback 四键），
 *     下次委派按 mtime 热加载生效
...
export const name = 'dsh-preset-flash-director'
...
export function apply(ctx) {
  // 预设自动同步：插件安装/更新即部署 agent 预设（异步，不阻塞插件注册）
  pluginVersion()
    .then((v) => syncPreset({ pluginVersion: v }))
    .then((r) => {
      if (r.action === 'install') console.log(`[flash-director-ui] agent preset installed -> ${r.target}`)
      else if (r.action === 'update') console.log(`[flash-director-ui] agent preset updated -> ${r.target}（备份 ${r.backup}）`)
      else if (r.action === 'adopt') console.log(`[flash-director-ui] adopted existing preset at ${r.target}`)
    })
    .catch((e) => console.warn(`[flash-director-ui] preset sync failed: ${e && e.message ? e.message : String(e)}`))
