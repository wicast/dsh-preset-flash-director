/**
 * dsh-preset-flash-director — agent.cordis.yml 基线行级定位/读取/打补丁（纯函数层）
 *
...
// CRLF 容错：Windows 检出/生成的文件可能是 \r\n 结尾。逐行剥掉行尾 \r，
// 让后续正则（如 `(.*)$`）在干净的行上工作；输出侧再由调用方按原 EOL 还原。
function stripCr(lines) {
...
export function patchCordisBaseline(text, patch) {
  ...
  // CRLF 容错：探测原文件 EOL，按 /\r?\n/ 切行后逐字处理，最后用原 EOL 还原（不改动换行风格）
  ...
  return { ok: true, text: out.join(eol), changed }
}
